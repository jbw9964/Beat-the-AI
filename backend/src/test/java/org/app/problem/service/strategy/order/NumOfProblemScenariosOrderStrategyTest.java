...
@Slf4j
@Import(NumOfProblemScenariosOrderStrategyTest.DataInitFacade.class)
@SuppressWarnings("SpringJavaInjectionPointsAutowiringInspection")
class NumOfProblemScenariosOrderStrategyTest extends IntegrationTestSupport {

    @Autowired
    NumOfProblemScenariosAscOrderStrategy ascendingStrategy;

    @Autowired
    NumOfProblemScenariosDescOrderStrategy descendingStrategy;
    ...
    @Test
    @DisplayName("문제 시나리오 개수 오름차순 정렬 검색이 정상 작동한다.")
    void testAscendingOrder() {
        ...
        Utils.assertListHasSizeAndSortedAccordingToOrder(
                testSize, searchResult, nopsAscAndNullLast
        );
    }

    @Test
    @DisplayName("문제 시나리오 개수 내림차순 정렬 검색이 정상 작동한다.")
    void testDecendingOrder() {
        ...
        Utils.assertListHasSizeAndSortedAccordingToOrder(
                testSize, searchResult, nopsDescAndNullLast
        );
    }
