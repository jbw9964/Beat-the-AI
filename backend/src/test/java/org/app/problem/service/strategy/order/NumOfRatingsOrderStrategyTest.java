...
@Slf4j
@Import(NumOfRatingsOrderStrategyTest.DataInitFacade.class)
@SuppressWarnings("SpringJavaInjectionPointsAutowiringInspection")
class NumOfRatingsOrderStrategyTest extends IntegrationTestSupport {

    @Autowired
    NumOfRatingsAscOrderStrategy ascendingStrategy;

    @Autowired
    NumOfRatingsDescOrderStrategy descendingStrategy;
    ...
    @Test
    @DisplayName("문제 평가 개수 오름차순 정렬 검색이 정상 작동한다.")
    void testAscendingOrder() {
        ...
        Comparator<Problem> norAscAndNullLast = Comparator.comparing(
                p -> p.getProblemAggregation().getRatingInfo().getNumOfTotalRatings(),
    ...
    @Test
    @DisplayName("문제 평가 개수 내림차순 정렬 검색이 정상 작동한다.")
