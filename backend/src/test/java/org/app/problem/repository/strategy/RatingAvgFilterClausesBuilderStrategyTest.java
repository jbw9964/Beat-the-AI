...
@Slf4j
@Import(RatingAvgFilterClausesBuilderStrategyTest.DataInitFacade.class)
@SuppressWarnings("SpringJavaInjectionPointsAutowiringInspection")
class RatingAvgFilterClausesBuilderStrategyTest extends IntegrationTestSupport {

    @Autowired
    RatingAvgFilterClausesBuilderStrategy strategy;
    ...
    @Test
    @DisplayName("두 평균 평가 점수 사이 존재한는 문제를 검색할 수 있다.")
    void testBoundedCase() {
        ...
        ProblemFilter<?> between1020Filter = Utils.genProblemFilter(
                strategy, (double) l10, (double) l20, null
        );
        ProblemFilter<?> between3040Filter = Utils.genProblemFilter(
                strategy, (double) l30, (double) l40, null
        );
        ...
        Utils.assertAllElementsSatisfies(
                searchResult1, p -> {
                    double score = p.getProblemAggregation()
                            .getRatingInfo()
                            .getRatingAverage();

                    return l10 <= score && score <= l20;
                }
        );
        ...
        Utils.assertAllElementsSatisfies(
                searchResult1, p -> {
                    ...
                    return l30 <= score && score <= l40;
                }
        );
    }

    @Test
    @DisplayName("어느 평균 평가 점수 기준 존재하는 문제를 검색할 수 있다.")
    void testUnboundedCase() {
        ...
        ProblemFilter<?> lowerFilter = Utils.genProblemFilter(
                strategy, null, (double) threshold, null
        );
        ProblemFilter<?> upperFilter = Utils.genProblemFilter(
                strategy, (double) threshold, null, null
        );
