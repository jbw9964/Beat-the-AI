...
@Slf4j
@Import(NumOfPlaysFilterClausesBuilderStrategyTest.DataInitFacade.class)
@SuppressWarnings("SpringJavaInjectionPointsAutowiringInspection")
class NumOfPlaysFilterClausesBuilderStrategyTest extends IntegrationTestSupport {

    @Autowired
    NumOfPlaysFilterClausesBuilderStrategy strategy;
    ...
    @Test
    @DisplayName("두 횟수 사이 플레이된 문제를 검색할 수 있다.")
    ...
    @Test
    @DisplayName("어느 횟수 기준 플레이된 문제를 검색할 수 있다.")
    void testUnboundedCase() {
        ...
        Utils.assertAllElementsSatisfies(
                searchResult1,
                p -> p.getProblemAggregation().getPlayInfo().getNumOfTotalPlays() <= threshold
        );
    ...
    private List<Problem> prepareData(
        ...
        for (int i = 0; i < testSize; i++) {
            ...
            AggregatedProblemPlayInfo playInfo = new AggregatedProblemPlayInfo(rand);
